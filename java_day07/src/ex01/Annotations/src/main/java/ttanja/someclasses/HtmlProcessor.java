...
public class HtmlProcessor extends FormAbstractProcessor {
    @Override
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (Element userForm : roundEnv.getElementsAnnotatedWith(HtmlForm.class)) {
            ...
            for (Element field : roundEnv.getElementsAnnotatedWith(HtmlInput.class)) {
                ...
                line += "\t<input type = " + htmlInputAnn.type() + "\" name = \"" + htmlInputAnn.name() + "\" placeholder = \"" + htmlInputAnn.placeholder() + "\">\n";
            }

            line += "\t<input type = \"submit\" value = \"Send\">\n</form>";
            try (BufferedWriter writter = new BufferedWriter(new FileWriter(System.getProperty("user.dir") + "/src/ex01/Annotations/target/classes/" + htmlFormAnn.fileName()))) {
                writter.write(line);
